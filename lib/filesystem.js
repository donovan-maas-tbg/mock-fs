...
const isWindowsOrig = isWindows;

// on Win32, change filepath from \\?\c:\a\b to C:\a\b
function getRealPathRaw(filepath, isWindows) {
  if (isWindows && (filepath.startsWith('\\\\?\\') || filepath.startsWith('\\??\\'))) {
    // Remove win32 file namespace prefix \\?\ or \??\
...
function getRealPath(filepath) {
  return getRealPathRaw(filepath, isWindows);
}

function getRealPathForceWin32(filepath) {
  return getRealPathRaw(filepath, true);
}
...
function getPathParts(filepath) {
  ...
  const namespacedPath = path.toNamespacedPath(resolvedPath);
  const parts = namespacedPath.split(path.sep);
  parts.shift();
  if (isWindows) {
 ...
 *     (defaults to `true`).
 * @param {boolean} options.emulateWin32 Emulates Win32 paths
 *     (defaults to `false`).
 * @class
 */
function FileSystem(options) {
  ...
  const emulateWin32 = 'emulateWin32' in options ? options.emulateWin32 : false;

  //   When running on a non-win32 system, and we want to emulate
  // functionality of a win32 system (such as for unit testing), replace
  // the `path` import with the Win32 specific version, and replace 
  // `getRealPath` with a version that always acts like it is running on
  // Win32.
  if (emulateWin32 && !isWindows) {
    this.emulateWin32 = emulateWin32;
    path = pathRaw.win32;
    module.exports.getRealPath = getRealPathForceWin32;
    isWindows = true;
  }
  else {
    this.emulateWin32 = false;
  }
  ...
  if (isWindows) {
    for (let i = "A".charCodeAt(0); i <= "Z".charCodeAt(0); ++i) {
      defaults.push("\\\\?\\" + String.fromCharCode(i) + ":");
    }
    defaults.push("\\\\?\\UNC")
  }
